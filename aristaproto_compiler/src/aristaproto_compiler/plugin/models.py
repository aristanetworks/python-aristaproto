...
from aristaproto_compiler import casing
from aristaproto_compiler.compile.importing import get_type_reference, parse_source_type_name
from aristaproto_compiler.compile.naming import (
...
class FieldCompiler(ProtoContentBase):
    ...
    def get_field_string(self) -> str:
        ...
        name = f"{self.py_name}"
        field_args = ", ".join(([""] + self.aristaproto_field_args) if self.aristaproto_field_args else [])

        aristaproto_field_type = (
            f"aristaproto.field({self.proto_obj.number}, aristaproto.TYPE_{str(self.field_type)}{field_args})"
        )
        if self.py_name in dir(builtins):
            self.message.builtins_types.add(self.py_name)
        return f'{name}: "{self.annotation}" = {aristaproto_field_type}'
    ...
    def annotations(self) -> list[str]:
        ...
        if self.proto_obj.type in (FieldType.INT32, FieldType.SFIXED32, FieldType.SINT32):
            ...
            annotations.append("pydantic.AfterValidator(aristaproto.validators.validate_float32)")

        elif self.proto_obj.type == FieldType.STRING:
            annotations.append("pydantic.AfterValidator(aristaproto.validators.validate_string)")
...
class MapEntryCompiler(FieldCompiler):
    ...
    def get_field_string(self) -> str:
        """Construct string representation of this field as a field."""
        proto_type_1 = f"aristaproto.TYPE_{self.proto_k_type}"
        proto_type_2 = f"aristaproto.TYPE_{self.proto_v_type}"
        ...
        aristaproto_field_type = (
            ...
            "aristaproto.TYPE_MAP, "
            f"map_meta=aristaproto.map_meta({proto_type_1}, {proto_type_2}{unwrap_2})"
            ")"
        )
        if self.py_name in dir(builtins):
            self.message.builtins_types.add(self.py_name)
        return f'{self.py_name}: "{self.annotation}" = {aristaproto_field_type}'
